...
async function getFirebaseAccessToken(): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get Firebase access token: ${await response.text()}`,
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (role === 'admin') {
      ...
      console.log(
        `🔍 Admin token user_ids:`,
        adminTokens?.map((t) => t.user_id),
      );
      ...
      if (!adminTokens || adminTokens.length === 0) {
        ...
        console.log(
          `🔍 Enabled admin settings found: ${enabledAdmins?.length || 0}`,
        );
        console.log(
          `🔍 Enabled admin user_ids:`,
          enabledAdmins?.map((s) => s.user_id),
        );
        ...
        tokens = adminTokens.filter((token) => {
          ...
          console.log(
            `🔍 Token for user ${token.user_id}: enabled=${isEnabled}`,
          );
          return isEnabled;
        });
      ...
      if (!settings || !settings.push_notifications) {
        console.log(
          `⏭️ NOTIFICATION SKIPPED → ${recipientInfo} (push notifications disabled)`,
        );
        return new Response(
          JSON.stringify({
            success: true,
    ...
    console.log(
      `📨 NOTIFICATION SENT → ${recipientInfo} | Success: ${successCount}/${tokens.length} | Title: "${title}"`,
    );
